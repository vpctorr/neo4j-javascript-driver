...
const TrustStrategy = {
    TRUST_CUSTOM_CA_SIGNED_CERTIFICATES: async function (config) {
        if (!config.trustedCertificates ||
            ...
            throw newError('You are using TRUST_CUSTOM_CA_SIGNED_CERTIFICATES as the method ' +
        ...
        const caCerts = await Promise.all(config.trustedCertificates.map(f => Deno.readTextFile(f)));
    ...
    TRUST_ALL_CERTIFICATES: function (config) {
        throw newError(`"${config.trust}" is not available in DenoJS. ` +
...
async function _connect(config) {
    ...
    throw newError('Unknown trust strategy: ' +
        config.trust +
        '. Please use either ' +
