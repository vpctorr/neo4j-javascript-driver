...
const { util: { ENCRYPTION_OFF, ENCRYPTION_ON, isEmptyObjectOrNull } } = internal;
...
function _connect(config, onSuccess, onFailure = () => null) {
    ...
    if (!isEncrypted(config)) {
        ...
        onFailure(newError('Unknown trust strategy: ' +
            config.trust +
            '. Please use either ' +
...
export default class NodeChannel {
    ...
    setupReceiveTimeout(receiveTimeout) {
        ...
        this._conn.on('timeout', () => {
            this._conn.destroy(newError(`Connection lost. Server didn't respond in ${receiveTimeout}ms`, this._connectionErrorCode));
        });
