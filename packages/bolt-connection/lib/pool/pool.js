...
class Pool {
     ...
     */
    constructor({ create = (acquisitionContext, address, release) => Promise.resolve(), destroy = conn => Promise.resolve(), validateOnAcquire = (acquisitionContext, conn) => true, validateOnRelease = (conn) => true, installIdleObserver = (conn, observer) => { }, removeIdleObserver = conn => { }, config = PoolConfig.defaultConfig(), log = Logger.noOp() } = {}) {
        this._create = create;
