...
import { ResultStreamObserver, ProcedureRouteObserver } from './stream-observers';
import transformersFactories from './bolt-protocol-v4x0.transformer';
import Transformer from './transformer';
import { internal } from 'neo4j-driver-core';
const { 
// eslint-disable-next-line no-unused-vars
bookmarks: { Bookmarks }, constants: { BOLT_PROTOCOL_V4_0, FETCH_ALL }, txConfig: { TxConfig } } = internal;
...
export default class BoltProtocol extends BoltProtocolV3 {
    ...
    run(query, parameters, { bookmarks, txConfig, database, impersonatedUser, notificationFilter, mode, beforeKeys, afterKeys, beforeError, afterError, beforeComplete, afterComplete, flush = true, reactive = false, fetchSize = FETCH_ALL, highRecordWatermark = Number.MAX_VALUE, lowRecordWatermark = Number.MAX_VALUE } = {}) {
        const observer = new ResultStreamObserver({
            server: this._server,
            reactive,
            fetchSize,
