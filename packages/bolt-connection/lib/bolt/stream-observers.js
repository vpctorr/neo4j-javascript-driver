 ...
 */
import { newError, error, 
// eslint-disable-next-line no-unused-vars
Integer, Record, json, internal } from 'neo4j-driver-core';
import RawRoutingTable from './routing-table-raw';
const { constants: { FETCH_ALL } } = internal;
...
class ResultStreamObserver extends StreamObserver {
     ...
     */
    constructor({ reactive = false, moreFunction, discardFunction, fetchSize = FETCH_ALL, beforeError, afterError, beforeKeys, afterKeys, beforeComplete, afterComplete, server, highRecordWatermark = Number.MAX_VALUE, lowRecordWatermark = Number.MAX_VALUE } = {}) {
        super();
    ...
    _handlePullSuccess(meta) {
        ...
        if (![undefined, null, 'r', 'w', 'rw', 's'].includes(completionMetadata.type)) {
            this.onError(newError(`Server returned invalid query type. Expected one of [undefined, null, "r", "w", "rw", "s"] but got '${completionMetadata.type}'`, PROTOCOL_ERROR));
            return;
        }
        this._setState(_states.SUCCEEDED);
...
class LogoffObserver extends StreamObserver {
    ...
    onNext(record) {
        this.onError(newError('Received RECORD when logging off ' + json.stringify(record)));
    }
    onError(error) {
        if (this._onError) {
            this._onError(error);
...
class TelemetryObserver extends ResultStreamObserver {
    ...
    onNext(record) {
        this.onError(newError('Received RECORD when sending telemetry ' + json.stringify(record), PROTOCOL_ERROR));
    }
    onError(error) {
        if (this._onError) {
            this._onError(error);
    ...
    onCompleted() {
        if (this._records !== null && this._records.length !== 1) {
            this.onError(newError('Illegal response from router. Received ' +
                ...
                json.stringify(this._records), PROTOCOL_ERROR));
            return;
        }
        if (this._onCompleted) {
...
export { StreamObserver, ResultStreamObserver, LoginObserver, LogoffObserver, ResetObserver, FailedObserver, CompletedObserver, RouteObserver, ProcedureRouteObserver, TelemetryObserver };
