 ...
 */
import { isPoint, int, isDuration, Duration, isLocalDateTime, isLocalTime, internal, isTime, Time, isDate, isDateTime, DateTime, Point, isInt } from 'neo4j-driver-core';
import { structure } from '../packstream';
import { TypeTransformer } from './transformer';
import { epochDayToDate, nanoOfDayToLocalTime, epochSecondAndNanoToLocalDateTime } from './temporal-factory';
import v1 from './bolt-protocol-v1.transformer';
const { temporalUtil: { dateToEpochDay, localDateTimeToEpochSecond, localTimeToNanoOfDay } } = internal;
const POINT_2D = 0x58;
...
function createLocalDateTimeTransformer({ disableLosslessIntegers, useBigInt }) {
    return new TypeTransformer({
        ...
        fromStructure: struct => {
            structure.verifyStructSize('LocalDateTime', LOCAL_DATE_TIME_STRUCT_SIZE, struct.size);
...
function createDateTimeWithZoneIdTransformer({ disableLosslessIntegers, useBigInt }) {
    return new TypeTransformer({
        ...
        fromStructure: struct => {
            structure.verifyStructSize('DateTimeWithZoneId', DATE_TIME_WITH_ZONE_ID_STRUCT_SIZE, struct.size);
...
function createDateTimeWithOffsetTransformer({ disableLosslessIntegers, useBigInt }) {
    return new TypeTransformer({
        ...
        fromStructure: struct => {
            structure.verifyStructSize('DateTimeWithZoneOffset', DATE_TIME_WITH_ZONE_OFFSET_STRUCT_SIZE, struct.size);
