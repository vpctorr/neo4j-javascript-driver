...
const { constants: { ACCESS_MODE_READ, FETCH_ALL }, util: { assertString } } = internal;
...
export default class RequestMessage {
    ...
    static run(query, parameters) {
        return new RequestMessage(RUN, [query, parameters], () => `RUN ${query} ${json.stringify(parameters)}`);
    }
    /**
    ...
    static hello5x2(userAgent, notificationFilter = null, routing = null) {
        ...
        return new RequestMessage(HELLO, [metadata], () => `HELLO ${json.stringify(metadata)}`);
    }
    /**
     * Create a new HELLO message.
     * @param {string} userAgent the user agent.
    ...
    static runWithMetadata(query, parameters, { bookmarks, txConfig, database, mode, impersonatedUser, notificationFilter } = {}) {
        const metadata = buildTxMetadata(bookmarks, txConfig, database, mode, impersonatedUser, notificationFilter);
        return new RequestMessage(RUN, [query, parameters, metadata], () => `RUN ${query} ${json.stringify(parameters)} ${json.stringify(metadata)}`);
    }
    /**
    ...
    static pull({ stmtId = NO_STATEMENT_ID, n = FETCH_ALL } = {}) {
        const metadata = buildStreamMetadata(stmtId === null || stmtId === undefined ? NO_STATEMENT_ID : stmtId, n || FETCH_ALL);
        return new RequestMessage(PULL, [metadata], () => `PULL ${json.stringify(metadata)}`);
    }
    /**
    ...
    static discard({ stmtId = NO_STATEMENT_ID, n = FETCH_ALL } = {}) {
        const metadata = buildStreamMetadata(stmtId === null || stmtId === undefined ? NO_STATEMENT_ID : stmtId, n || FETCH_ALL);
        return new RequestMessage(DISCARD, [metadata], () => `DISCARD ${json.stringify(metadata)}`);
    }
    ...
    static route(routingContext = {}, bookmarks = [], databaseName = null) {
        return new RequestMessage(ROUTE, [routingContext, bookmarks, databaseName], () => `ROUTE ${json.stringify(routingContext)} ${json.stringify(bookmarks)} ${databaseName}`);
    }
    /**
     * Generate the ROUTE message, this message is used to fetch the routing table from the server
     *
    ...
    static routeV4x4(routingContext = {}, bookmarks = [], databaseContext = {}) {
        ...
        return new RequestMessage(ROUTE, [routingContext, bookmarks, dbContext], () => `ROUTE ${json.stringify(routingContext)} ${json.stringify(bookmarks)} ${json.stringify(dbContext)}`);
    }
}
/**
