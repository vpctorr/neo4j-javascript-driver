 ...
 */
import { Node, newError, error, Relationship, UnboundRelationship, Path, toNumber, PathSegment } from 'neo4j-driver-core';
...
function createNodeTransformer() {
    return new TypeTransformer({
        ...
        toStructure: object => {
            throw newError(`It is not allowed to pass nodes in query parameters, given: ${object}`, PROTOCOL_ERROR);
        },
...
function createRelationshipTransformer() {
    return new TypeTransformer({
        ...
        toStructure: object => {
            throw newError(`It is not allowed to pass relationships in query parameters, given: ${object}`, PROTOCOL_ERROR);
        },
        fromStructure: struct => {
...
function createUnboundRelationshipTransformer() {
    return new TypeTransformer({
        ...
        toStructure: object => {
            throw newError(`It is not allowed to pass unbound relationships in query parameters, given: ${object}`, PROTOCOL_ERROR);
        },
        fromStructure: struct => {
            structure.verifyStructSize('UnboundRelationship', UNBOUND_RELATIONSHIP_STRUCT_SIZE, struct.size);
...
function createPathTransformer() {
    return new TypeTransformer({
        ...
        toStructure: object => {
            throw newError(`It is not allowed to pass paths in query parameters, given: ${object}`, PROTOCOL_ERROR);
        },
        fromStructure: struct => {
