...
export default class RxRetryLogic {
     ...
     */
    constructor({ maxRetryTimeout = DEFAULT_MAX_RETRY_TIME_MS, initialDelay = DEFAULT_INITIAL_RETRY_DELAY_MS, delayMultiplier = DEFAULT_RETRY_DELAY_MULTIPLIER, delayJitter = DEFAULT_RETRY_DELAY_JITTER_FACTOR, logger = null } = {}) {
     ...
     */
    retry(work) {
        return work.pipe(retryWhen(failedWork => {
            const handledExceptions = [];
            const startTime = Date.now();
            let retryCount = 1;
            let delayDuration = this._initialDelay;
            return failedWork.pipe(mergeMap(err => {
                ...
                if (retryCount >= 2 &&
                    ...
                    const error = newError(`Failed after retried for ${retryCount} times in ${this._maxRetryTimeout} ms. Make sure that your database is online and retry again.`, SERVICE_UNAVAILABLE);
                ...
                retryCount++;
                if (this._logger) {
                    this._logger.warn(`Transaction failed and will be retried in ${nextDelayDuration}`);
                }
