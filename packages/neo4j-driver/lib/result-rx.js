...
export default class RxResult {
    ...
    constructor(result, state) {
        ...
        this._keys = replayedResult.pipe(mergeMap(r => from(r.keys())), publishReplay(1), refCount());
     ...
     */
    records() {
        const result = this._result.pipe(mergeMap(result => new Observable(recordsObserver => this._startStreaming({ result, recordsObserver }))));
     ...
     */
    consume() {
        return this._result.pipe(mergeMap(result => new Observable(summaryObserver => this._startStreaming({ result, summaryObserver }))));
    }
    /**
    ...
    _startStreaming({ result, recordsObserver = null, summaryObserver = null } = {}) {
        const subscriptions = [];
        if (summaryObserver) {
        ...
        if (this._state < States.STREAMING) {
            ...
            recordsObserver.error(newError('Streaming has already started/consumed with a previous records or summary subscription.'));
        }
        return () => {
...
function createFullyControlledSubject(iterator, completeObserver, streamControl = new StreamControl()) {
