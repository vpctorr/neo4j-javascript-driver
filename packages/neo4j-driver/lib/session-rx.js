...
const { constants: { ACCESS_MODE_READ, ACCESS_MODE_WRITE, TELEMETRY_APIS }, txConfig: { TxConfig } } = internal;
/**
...
export default class RxSession {
    ...
    run(query, parameters, transactionConfig) {
        return new RxResult(new Observable(observer => {
            try {
        ...
        return new Observable(observer => {
            try {
                this._session._beginTransaction(accessMode, txConfig, apiTelemetryConfig)
                    .then(tx => {
                    observer.next(new RxTransaction(tx));
                    observer.complete();
                })
                    .catch(err => observer.error(err));
            }
    ...
    _runTransaction(accessMode, work, transactionConfig, transactionWrapper = (tx) => tx) {
        ...
        return this._retryLogic.retry(of(1).pipe(mergeMap(() => this._beginTransaction(accessMode, txConfig, context.apiTelemetryConfig)), mergeMap(txc => defer(() => {
            try {
        ...
        }).pipe(catchError(err => txc.rollback().pipe(concatWith(throwError(() => err)))), concatWith(txc.commit())))));
    }
}
function _createRetryLogic(config) {
    const maxRetryTimeout = config && config.maxTransactionRetryTime
